...
def set_shoot() -> shoot.Shoot:
    ...
    sim_params = params.SunlitShadedParams(
    ...
    canopy = shoot.Shoot(
        leaves_category="sunlit-shaded", inputs=sim_inputs, params=sim_params
    )
...
def plot_results(canopy: shoot.Shoot):
    ...
    for index in layer_indexes:
        ...
        sunlit_beam.append(
            (
                canopy[index].abs_direct_by_sunlit,
                canopy[index].thickness * canopy[index].sunlit_fraction,
            )
        )
        sunlit_diffuse.append(
            (
                canopy[index].abs_diffuse_by_sunlit,
                canopy[index].thickness * canopy[index].sunlit_fraction,
            )
        )
        sunlit_scattered.append(
            (
                canopy[index].abs_scattered_by_sunlit,
                canopy[index].thickness * canopy[index].sunlit_fraction,
            )
        )
        shaded_diffuse.append(
            (
                canopy[index].abs_diffuse_by_shaded,
                canopy[index].thickness * canopy[index].shaded_fraction,
            )
        )
        shaded_scattered.append(
            (
                canopy[index].abs_scattered_by_shaded,
                canopy[index].thickness * canopy[index].shaded_fraction,
            )
        )
    ...
    axs[0, 0].plot(
        [v[0] for v in sunlit_beam],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{ground}}$",
    )
    axs[0, 0].plot(
        [v[0] / v[1] for v in sunlit_beam],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{leaf}}$",
    )

    axs[0, 1].plot(
        [v[0] for v in sunlit_diffuse],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{ground}}$",
    )
    axs[0, 1].plot(
        [v[0] / v[1] for v in sunlit_diffuse],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{leaf}}$",
    )

    axs[0, 2].plot(
        [v[0] for v in sunlit_scattered],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{ground}}$",
    )
    axs[0, 2].plot(
        [v[0] / v[1] for v in sunlit_scattered],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{leaf}}$",
    )

    axs[1, 1].plot(
        [v[0] for v in shaded_diffuse],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{ground}}$",
    )
    axs[1, 1].plot(
        [v[0] / v[1] for v in shaded_diffuse],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{leaf}}$",
    )

    axs[1, 2].plot(
        [v[0] for v in shaded_scattered],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{ground}}$",
    )
    axs[1, 2].plot(
        [v[0] / v[1] for v in shaded_scattered],
        layer_indexes,
        label=r"$\mathregular{W\/m^{-2}_{leaf}}$",
    )
    ...
    axs[0, 0].set_xlabel(
        r"$\mathregular{I_{abs,\/direct}}$" + "\n" + r"$\mathregular{[W \cdot m^{-2}]}$"
    )
    axs[1, 1].set(
        xlabel=r"$\mathregular{I_{abs,\/sky-diffused}}$"
        + "\n"
        + r"$\mathregular{[W \cdot m^{-2}]}$",
        ylabel="layer index [-]",
    )
    axs[1, 2].set_xlabel(
        r"$\mathregular{I_{abs,\/leaf-scattered}}$"
        + "\n"
        + r"$\mathregular{[W \cdot m^{-2}]}$"
    )
