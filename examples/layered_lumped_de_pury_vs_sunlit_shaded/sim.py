...
def generate_plots(
    ...
    lumped_canopy: shoot.Shoot,
    sunlit_shaded_canopy: shoot.Shoot,
):
    layer_indexes, lumped_layer_irradiance = zip(
        *[
            (index, layer.absorbed_irradiance["lumped"])
            for index, layer in lumped_canopy.items()
        ]
    )
    ...
    plot_absorbed_irradiance_profiles(
        incident_diffuse_irradiance,
        incident_direct_irradiance,
        layer_indexes,
        lumped_layer_irradiance,
        shaded_layer_irradiance,
        sunlit_layer_irradiance,
    )
...
def plot_absorbed_irradiance_profiles(
    incident_diffuse_irradiance,
    incident_direct_irradiance,
    layer_indexes,
    lumped_layer_irradiance,
    ...
    ax.vlines(
        incident_direct_irradiance,
        ymin=min(layer_indexes),
        ymax=max(layer_indexes),
...
def plot_linear_comparison(
    ...
    ax.plot(
        ...
        [
            sum(abs_par)
            for abs_par in zip(sunlit_layer_irradiance, shaded_layer_irradiance)
        ],
        "o-",
    )
    ax.set(
        xlabel=r"$\mathregular{Lumped\/per\/layer\/[W \cdot m^{-2}_{ground}]}$",
